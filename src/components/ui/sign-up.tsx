...
export const AuthComponent = ({ logo = <DefaultLogo />, brandName = "EaseMize", onSubmit, isLogin = false }: AuthComponentProps) => {
  ...
  const handleFinalSubmit = async (e: React.FormEvent) => {
    ...
    // For login mode, submit at password step
    ...
    // For signup mode, need confirm password
    if (!isLogin && authStep === 'confirmPassword') {
      ...
      if (onSubmit) {
        ...
        if (result.success) {
          ...
          setModalErrorMessage(result.error || "Sign up failed");
          setModalStatus('error');
        }
      }
    }
            ...
            <fieldset disabled={modalStatus !== 'closed'} className="relative z-10 flex flex-col items-center gap-8 w-[280px] mx-auto p-4">
                <AnimatePresence mode="wait">
                    {authStep === "email" && <motion.div key="email-content" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center gap-4">
                        <BlurFade delay={0.25 * 1} className="w-full"><div className="text-center"><p className="font-serif font-light text-4xl sm:text-5xl md:text-6xl tracking-tight text-foreground whitespace-nowrap">{isLogin ? "Welcome back" : "Get started with Us"}</p></div></BlurFade>
                    ...
                    {authStep === "password" && <motion.div key="password-title" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center text-center gap-4">
                        <BlurFade delay={0} className="w-full"><div className="text-center"><p className="font-serif font-light text-4xl sm:text-5xl tracking-tight text-foreground whitespace-nowrap">{isLogin ? "Enter password" : "Create your password"}</p></div></BlurFade>
                        <BlurFade delay={0.25 * 1}><p className="text-sm font-medium text-muted-foreground">{isLogin ? "Welcome back! Enter your password to continue." : "Your password must be at least 6 characters long."}</p></BlurFade>
                    </motion.div>}
